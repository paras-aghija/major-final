    ...
    */
    ...
    it("Transaction reverted: Remove the only signer", async () => {
      ...
      let removeSignerHash = await getRemoveSignerHash(
        ...
        const response = await MultiSigWallet.executeTransaction(
            MultiSigWallet.address,
            "0x0",
    ...
    it("Transaction reverted: Invalid MultiSigWallet, more signatures required than signers", async () => {
      ...
      expect(res).to.be.revertedWith("NOT_ENOUGH_SIGNERS()");
    });
    ...
    it("Transaction reverted: Update Signatures Required to 2 - trying to lock all the funds in the wallet, becasuse there is only 1 signer", async () => {
      ...
      let callData = await MultiSigWallet.interface.encodeFunctionData(
      ...
      const res1 = await MultiSigWallet.recover(hash, signature)
      expect(res1).to.equal(
        owner.address
      );

      const res = await MultiSigWallet.executeTransaction(to, value, callData, [signature]);
